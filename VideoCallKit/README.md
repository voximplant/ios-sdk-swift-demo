...
* [MainViewController.swift](Stories/Main/MainViewController.swift) uses `CXCallController` to report outgoing calls
* [CallViewController.swift](Stories/Call/CallViewController.swift) uses `CXCallController` to request `CXTransactions` to change the call state (mute, hold, video, call end by the current user)
